    ...
    module CouchRestModel
      ...
      module Schema
        ...
        def find(*args)
          ...
          if options.present?
            if options[:conditions].has_key?(:remember_token)
              find_one_by_key_and_value("remember_token", options[:conditions][:remember_token])
              #raise "what's the fuck options? #{options} | #{options[:conditions].class}"
            else
